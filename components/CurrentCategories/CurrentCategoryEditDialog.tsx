...
interface CurrentCategoryEditDialogProps {
  category: CurrentCategory | null;
  isOpen: boolean;
  onClose: () => void;
}

const CurrentCategoryEditDialog: React.FC<CurrentCategoryEditDialogProps> = ({
  category,
  isOpen,
  onClose,
}) => {
    ...
    const fetchCategories = async () => {
      try {
        ...
        const filteredCategories = data.filter(
          (cat: CurrentCategory) => cat.id !== category?.id
        );
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Failed to update current category");
      }

      toast({
        title: "Success",
        description: "Current category updated successfully",
      });
      onClose();
      ...
      toast({
        ...
        description:
          err instanceof Error
            ? err.message
            : "Failed to update current category",
      });
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Failed to delete current category");
      }

      toast({
        title: "Success",
        description: "Current category deleted successfully",
      });
      ...
      toast({
        ...
        description:
          err instanceof Error
            ? err.message
            : "Failed to delete current category",
      });
    }
  };
  ...
  return (
    <>
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Cari Kategori Düzenle</DialogTitle>
          </DialogHeader>
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Cari Kategoriyi Sil</AlertDialogTitle>
            <AlertDialogDescription>
              Bu cari kategoriyi silmek istediğinizden emin misiniz? Bu işlem
              geri alınamaz.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
export default CurrentCategoryEditDialog;
