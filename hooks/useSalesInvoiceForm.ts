...
import { z } from 'zod';
import { salesInvoiceSchema } from '@/lib/validations/salesInvoice';
import { useToast } from '@/hooks/use-toast';
import { InvoiceFormData } from '@/components/SalesInvoice/types';
...
export const useSalesInvoiceForm = (initialData?: Partial<InvoiceFormData>) => {
    ...
    const validateField = useCallback((field: FormFields, value: unknown) => {
        try {
            const fieldSchema = salesInvoiceSchema.shape[field];
            if (fieldSchema) {
    ...
    const validateForm = useCallback((): boolean => {
        try {
            salesInvoiceSchema.parse(state.formData);
